...
## المصطلحات التأسيسية
<abbr title="جاء الاسم من الشيء الذي يتم تدريب الشبكة على تعلمه بشكل صريح، هو الباقي يُرمز له بـ F(x)، فبدل أن نطلب منها ان تتعلم كافة التحويل المطلوب من الدخل حتى الخرج والذي يرمز له بـ H(x)، ستتعلم كل طبقة أو مجموعة من الطبقات F(x)=H(x)-x ثم تعيد بناء الخرج المطلوب بجمع الباقي F(x) مع الدخل x لتحصل بذلك على H(x) = F(x) + x، لذلك يسمى تعلم المتبقي.">تعلم المتبقي</abbr>

<abbr title="هي الدالة (يعبر عنها كطبقة أو مجموعة طبقات) التي تمثل الفرق بين الخرج المطلوب والمدخلات، ويرمز لها بـ F(x)، لذلك تكون دالة بمرجعية إلى دخلها.">دالة الباقي</abbr>

<abbr title="هي دالة (يعبر عنها كطبقة أو مجموعة طبقات) تشير إلى تحويل الدخل إلى خرج دون الإشارة الصريحة (مرجعية) إلى المدخلات.">دالة بدون مرجعية</abbr>

<abbr title="تشير إلى شبكة VGG المستخدمة في هذه الورقة كأساس لمقارنة العمق والتعقيد.">مجموعة الهندسة البصرية</abbr>

<abbr title="هو المسابقة ILSVRC-2015 لقياس أداء نماذج التعرف على الصور بناء على مجموعة بيانات ImageNet شبكة الصور.">تحدي التعرف البصري واسع النطاق لشبكة الصور لعام 2015</abbr>

<abbr title="هو مسابقة COCO-2015 للكشف عن الكائنات، وتجزئتها، وعنونتها في الصور وذلك باستخدام مجموعة بيانات ’الكائنات العامة في السياق‘.">تحدي الكائنات العامة في السياق 2015</abbr>

<abbr title="هي CIFAR-10 مجموعة بيانات لتصنيف الصور، استخدمها الباحثون لتقييم شبكات البواقي.">مجموعة بيانات المعهد الكندي للأبحاث المتقدمة بـ 10 أصناف</abbr>

<abbr title="تتعلم طبقات الشبكة العصبية تمثيلاً لنمط أو مجموعة من الأنماط الموجودة في بيانات التدريب، وكلما زاد عمق الشبكة تعلمت الطبقات الأعمق تمثيلات ذو هرمية أغنى، وتلعب هذه التمثيلات دور الأهم في أداء مهام التصنيف.">تمثيل</abbr>

<abbr title="مجموعة بيانات Microsoft COCO واسعة النطاق والمخصصة لاكتشاف الكائنات وتجزئتها، طورتها مايكروسوفت لتدريب وتقييم أداء الشبكات العصبية العميقة على مهام الرؤية الحاسوبية.">الكائنات العامة في السياق</abbr>

<abbr title="هي شبكات عصبية عميقة للغاية تستخدم ’تعلم المتبقي‘ مع وصلات مختصرة لتسهيل تحسين النماذج العميقة.
تتعلم كل مجموعة طبقات، المكونة لشبكات البواقي، جزء المتبقي من طرح الدخل من التحويل المطلوب، بدل أن تتعلم كافة التحويل، ثم تضيف إلى ما تعلمته الدخل عبر الوصلة المختصرة. يسمح هذا التصميم بتعميق الشبكات العصبية بشكل كبير دون التأثير سلباً على دقة التدريب.">شبكات البواقي</abbr>

<abbr title="Objects Detection عملية تحديد صنف لكل كائن موجود في الصور، بالإضافة إلى تحديد مربع يحيط بتلك الكائنات.">كشف الكائنات</abbr>

<abbr title="Segmentation هي تعيين صنف لكل بكسل في الصورة، ما يؤدي إلى تجزئة حدود الكائن بدقة بدلا من رسم مربع يحيطه فقط.">تجزئة</abbr>

<abbr title="Localization هو تحديد الكائن الرئيسي ومكان وجوده في الصورة باستخدام مربع للإحاطة به.">تحديد الموقع</abbr>

<abbr title="Vanishing Gradient يشير إلى تضاؤل قيم التدرج بشكل كبير مع الانتشار العكسي القادم من نهاية الشبكة، ما يجعل عملية التدريب صعباً للغاية، ويؤدي إلى بطء شديد في تعليم الطبقات الأولى أو عدم تعلمها على الإطلاق، وهي احدى الصعوبات الرئيسية في تدريب الشبكات العميقة.">تلاشي التدرج</abbr>

<abbr title="Exploding Gradient يشير إلى تضخم قيم التدرج بشكل مفرط مع الانتشار العكسي القادم من نهاية الشبكة، ما يجعل عملية التدريب غير مستقرة، ويؤدي إلى زعزعة استقرار التدريب ومنع التقارب، وهي مشكلة معروفة في تدريب الشبكات العميقة.">انفجار التدرج</abbr>

<abbr title="Convergence يشير إلى التقدم باتجاه نجاح عملية التدريب في تقليل الخطأ والوصول إلى حل مستقر أثناء تحسين أوزان الطبقات، أما عدم التقارب فيعني فشل ’المُحسِّن‘ optimizer في إيجاد حل جيد، تأتي هذه الورقة ’بتعلم المتبقي‘ لتسهيل التقارب في الشبكات العميقة.">التقارب</abbr>

<abbr title="Normalized Initialization يشير إلى إعطاء قيم ابتدائية عشوائية لأوزان الطبقات من توزع احتمالي طبيعي، ما يحافظ على استقرار قيم الأوزان بحيث لا تتلاشى أو تنفجر قيم التنشيط والتدرجات، ما يساعد الشبكة العميقة على بدء التقارب، وهي احدى العوامل التي جعلت تدريب الشبكات العميقة ممكناً.">التهيئة الطبيعية</abbr>

<abbr title="Intermediate Normalization Layers طبقة من نوع خاص تضاف بين طبقات الشبكة لتحسين استقرار التدريب ومساعدة الشبكة العميقة على التقارب، حيث تعمل على جعل خرج ’التنشيط‘ الطبقة الوسيطة يتبع للتوزيع الطبيعي.">طبقات التطبيع الوسيطة</abbr>

<abbr title="Stochastic Gradient Descent (SGD) طريقة تحسين تستخدم لتدريب الشبكات العصبية عبر ’الانتشار العكسي‘ حيث تُعدل أوزان الطبقات بالاعتماد على دفعات صغيرة من البيانات، يدمج هذا المحسّن مع الانتشار العكسي لتقليل خطأ التدريب.">التدرج العشوائي</abbr>

<abbr title="Backpropagation يستخدم لحساب التدرجات وتعديل أوزان الطبقات أثناء تدريب الشبكة، بحيث يبدأ بنشر التدرجات وتعديل أوزان الشبكة من نهايتها إلى بدايتها (عكسياً).">الانتشار العكسي</abbr>

<abbr title="Degradation Problem تحدث عندما يؤدي إضافة المزيد من الطبقات إلى الشبكة العصبية إلى زيادة خطأ التدريب، على عكس المتوقع. ولا يعود ذلك إلى ’فرط التخصيص‘ Overfitting الذي يظهر خطأ تدريب منخفض وخطأ اختبار مرتفع، بل إلى صعوبة تحسين عدد كبير من أوزان الطبقات المتراكمة.">مشكلة التدهور</abbr>

<abbr title="تقيس Training Accuracy دقة النموذج على بيانات التدريب وأثناء عملية التدريب. يشير انخفاض دقة التدريب (يعني ارتفاع خطأ التدريب) إلى صعوبة في التحسن.">دقة التدريب</abbr>

<abbr title="Identity Mapping Layer تعمل على أن يكون الخرج مساوياً للدخل، بمعنى تمرير الدخل x مباشرة إلى الخرج، ويتم ذلك عملياً من خلال وصلات الاختصار.">طبقة مُطابقة</abbr>
